...
public enum iTweenActionType { SCALE = 0, ROTATE = 1, BLINKEYE = 2, ROTATE360 = 3, ROTATE360FLIPY = 4, SHAKE = 5, PUNCH = 6, BLINK = 7, FLOURSCALE = 8, MyAnimation = 9 }
...
public class ActionManager : MonoBehaviour
{
	...
	void Start ()
	{
		switch (actionType) {
			...
			case iTweenActionType.BLINK:
				iTweenBlinkAction (0.2f);
				break;
			case iTweenActionType.FLOURSCALE:
				iTweenFlourScaleAction ();
				break;
            case iTweenActionType.MyAnimation:
                iTweenMyAnimation();
                break;
        }
    ...
    private void iTweenMyAnimation()
    {
        Hashtable tweenParams = new Hashtable();
        tweenParams.Add("scale", new Vector3(1.15f, 1.15f, 0));
	...
	private void iTweenFlourScaleAction() {
		Hashtable tweenParams = new Hashtable();
		tweenParams.Add ("scale", new Vector3 (1.0f, 1.0f, 0));
		tweenParams.Add ("time", 0.5f);
		tweenParams.Add ("easetype", iTween.EaseType.easeInBounce);
		tweenParams.Add ("looptype", iTween.LoopType.pingPong);
		iTween.ScaleTo(gameObject, tweenParams);
	...
	private void Blink() {
		if (gameObject.GetComponent<RectTransform> () != null) {
			if (gameObject.GetComponent<RectTransform> ().localScale.x == 0) {
				gameObject.GetComponent<RectTransform> ().localScale = new Vector3 (0.9f, 0.9f, 0.9f);
				iTweenBlinkAction (0.2f);
			} else {
				gameObject.GetComponent<RectTransform> ().localScale = new Vector3 (0, 0, 0);
				iTweenBlinkAction (0.2f);

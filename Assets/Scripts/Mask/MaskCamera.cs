...
public enum MaskType { PAINT = 0, ERASE = 1 }

public class MaskCamera : MonoBehaviour
{
    public MaskType maskType = MaskType.PAINT;
    [HideInInspector]
    ...
    public IEnumerator Start()
    {
        ...
        if (this.maskType == MaskType.PAINT)
        {
            rawImage = this.mask.GetComponent<RawImage>();
            image = this.mask.GetComponentInChildren<Image>();
        }
        else if (this.maskType == MaskType.ERASE)
        {
            image = this.mask.GetComponent<Image>();
        }
        ...
        {
            image.material.SetTexture("_MaskTex", rt);
        }
    ...
    public void Update()
    {
		...
		if (Input.GetMouseButton(0) && GameManager.instance.canDrawMask)
        {
            ...
            if (this.screenRect.Contains(v))
                this.newBrushHolePosition = new Vector2(this.baseWidth * (v.x - this.screenRect.xMin) / this.screenRect.width, this.baseHeight * (v.y - this.screenRect.yMin) / this.screenRect.height);
        }
    }
